...
let rec eval (e : expr) (cont : cont) (econt : econt) = 
    ...
    | Prim(ope, e1, e2) -> 
      eval e1 (fun v1 -> fun econt1 ->
          ...
          econt
    | Bang(str) ->
        let strList = [for c in str -> string c]
        let rec aux strList2 =
            match strList2 with
            | [] -> econt ()
            | head :: tail -> cont (Str head) (fun() -> aux tail)
        aux strList
    | BangN(str, n) ->
        let rec loop str2 amount =
            if amount > 0 then
                eval (Bang str2) cont (fun () -> loop str2 (amount - 1))
            else
                econt ()
        loop str n
