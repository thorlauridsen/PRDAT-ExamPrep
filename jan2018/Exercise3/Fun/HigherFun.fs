...
let rec eval (e : expr) (env : value env) : value =
    ...
    | Let(x, eRhs, letBody) -> 
      ...
      eval letBody letEnv
    | Enum(str, strList, expr) ->
        let enumEnv = (str, EVal(str, strList)) :: env
        eval expr enumEnv

    | EnumVal(enumName, str2) ->
        match lookup env enumName with
        | EVal(enumName, vs) ->
            try Int (List.findIndex ((=) str2) vs)
            with _ -> failwith ("EnumVal: unknown enum value " + enumName + "." + str2)
        | _ -> failwith ("EnumVal: unknown enum " + enumName)
