...
let rec eval (e : expr) (env : value env) : value =
    ...
    | Field (exp, str) ->
      ...
      | RecordV lst -> match List.tryFind (fun (str', _) -> str = str') lst with
          | None -> failwith ("Field " + str + " not found!")
          | Some (_, valu) -> valu
      | _ -> failwith ("Extreme fail")

    | Record(lst) ->
      RecordV (List.map (fun (str,exp) -> (str, eval exp env)) lst)
